...
class _SupplierViewState extends State<SupplierView> {
  ...
  Future<void> _refresh() async {
    ...
    for (var supplier in data) {
      final debtAmount = await db.getSupplierDebtAmount(supplier['name']);
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  const Text("TỔNG NHẬP:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey)),
                                  Text("${NumberFormat('#,###').format(s['totalAmount'])} đ", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey)),
                                ],
                              ),
                            ),
                            const SizedBox(height: 10),
