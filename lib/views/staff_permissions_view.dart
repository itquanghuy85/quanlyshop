...
class _StaffPermissionsViewState extends State<StaffPermissionsView> {
  ...
  Future<void> _updateUserPermission(
    ...
    try {
      ...
      final permissions = _buildEffectivePermissions(userData);
      permissions[permissionKey] = value;

      await UserService.updateUserPermissions(
        uid: uid,
        allowViewSales: permissions['allowViewSales'] ?? false,
        allowViewRepairs: permissions['allowViewRepairs'] ?? false,
        allowViewInventory: permissions['allowViewInventory'] ?? false,
        allowViewParts: permissions['allowViewParts'] ?? false,
        allowViewSuppliers: permissions['allowViewSuppliers'] ?? false,
        allowViewCustomers: permissions['allowViewCustomers'] ?? false,
        allowViewWarranty: permissions['allowViewWarranty'] ?? false,
        allowViewChat: permissions['allowViewChat'] ?? false,
        allowViewAttendance: permissions['allowViewAttendance'] ?? false,
        allowViewPrinter: permissions['allowViewPrinter'] ?? false,
        allowViewRevenue: permissions['allowViewRevenue'] ?? false,
        allowViewExpenses: permissions['allowViewExpenses'] ?? false,
        allowViewDebts: permissions['allowViewDebts'] ?? false,
        allowViewCostPrice: permissions['allowViewCostPrice'] ?? false,
        allowViewPurchaseOrders: permissions['allowViewPurchaseOrders'],
        allowCreatePurchaseOrders: permissions['allowCreatePurchaseOrders'],
        allowViewSettings: permissions['allowViewSettings'],
        allowManageStaff: permissions['allowManageStaff'],
      );
      if (mounted) {
        ...
        await AuditService.logAction(
          ...
          entityId: uid,
          summary:
              'Đã ${value ? 'bật' : 'tắt'} quyền ${permissionKey.replaceAll('allowView', '').toLowerCase()}',
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Đã cập nhật quyền ${permissionKey.replaceAll('allowView', '').toLowerCase()}",
            ),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Lỗi cập nhật quyền: $e"),
  ...
  Future<void> _updateUserRole(String uid, String newRole) async {
    try {
      ...
      final permissions = UserService.getDefaultPermissionsForRole(newRole);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Đã cập nhật vai trò thành ${newRole == 'owner'
                  ? 'Chủ shop'
                  : newRole == 'manager'
                  ? 'Quản lý'
                  : newRole == 'employee'
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Lỗi cập nhật vai trò: $e"),
            backgroundColor: Colors.red,
          ),
        );
      }
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                final effectivePermissions = _buildEffectivePermissions(
                  userData,
                );
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (role != 'owner' && role != 'manager') ...[
                              ...
                              _buildPermissionRow(
                                "BÁN HÀNG",
                                effectivePermissions['allowViewSales'] ?? false,
                                uid,
                                'allowViewSales',
                              ),
                              _buildPermissionRow(
                                "SỬA CHỮA",
                                effectivePermissions['allowViewRepairs'] ??
                                    false,
                                uid,
                                'allowViewRepairs',
                              ),
                              _buildPermissionRow(
                                "KHO HÀNG",
                                effectivePermissions['allowViewInventory'] ??
                                    false,
                                uid,
                                'allowViewInventory',
                              ),
                              _buildPermissionRow(
                                "LINH KIỆN SỬA CHỮA",
                                effectivePermissions['allowViewParts'] ?? false,
                                uid,
                                'allowViewParts',
                              ),
                              _buildPermissionRow(
                                "NHÀ CUNG CẤP",
                                effectivePermissions['allowViewSuppliers'] ??
                                    false,
                                uid,
                                'allowViewSuppliers',
                              ),
                              _buildPermissionRow(
                                "KHÁCH HÀNG",
                                effectivePermissions['allowViewCustomers'] ??
                                    false,
                                uid,
                                'allowViewCustomers',
                              ),
                              _buildPermissionRow(
                                "BẢO HÀNH",
                                effectivePermissions['allowViewWarranty'] ??
                                    false,
                                uid,
                                'allowViewWarranty',
                              ),
                              _buildPermissionRow(
                                "CHAT NỘI BỘ",
                                effectivePermissions['allowViewChat'] ?? false,
                                uid,
                                'allowViewChat',
                              ),
                              _buildPermissionRow(
                                "CHẤM CÔNG",
                                effectivePermissions['allowViewAttendance'] ??
                                    false,
                                uid,
                                'allowViewAttendance',
                              ),
                              _buildPermissionRow(
                                "CẤU HÌNH MÁY IN",
                                effectivePermissions['allowViewPrinter'] ??
                                    false,
                                uid,
                                'allowViewPrinter',
                              ),
                              ...
                              _buildPermissionRow(
                                "DOANH THU & LỜI LỖ",
                                effectivePermissions['allowViewRevenue'] ??
                                    false,
                                uid,
                                'allowViewRevenue',
                              ),
                              _buildPermissionRow(
                                "CHI PHÍ CỬA HÀNG",
                                effectivePermissions['allowViewExpenses'] ??
                                    false,
                                uid,
                                'allowViewExpenses',
                              ),
                              _buildPermissionRow(
                                "SỔ CÔNG NỢ",
                                effectivePermissions['allowViewDebts'] ?? false,
                                uid,
                                'allowViewDebts',
                              ),
                              _buildPermissionRow(
                                "GIÁ VỐN SẢN PHẨM",
                                effectivePermissions['allowViewCostPrice'] ??
                                    false,
                                uid,
                                'allowViewCostPrice',
                              ),
      ...
      child: Row(
        children: [
          ...
          Switch(
            value: value,
            onChanged: (newValue) =>
                _updateUserPermission(uid, permissionKey, newValue),
