...
class DBHelper {
  ...
  Future<Database> _initDB() async {
    ...
    final db = await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
          'CREATE TABLE IF NOT EXISTS suppliers(id INTEGER PRIMARY KEY AUTOINCREMENT, firestoreId TEXT UNIQUE, name TEXT, nameNorm TEXT, contactPerson TEXT, phone TEXT, email TEXT, address TEXT, note TEXT, items TEXT, importCount INTEGER DEFAULT 0, totalAmount INTEGER DEFAULT 0, active INTEGER DEFAULT 1, favorite INTEGER DEFAULT 0, type TEXT, createdAt INTEGER, updatedAt INTEGER, shopId TEXT, isSynced INTEGER DEFAULT 0, deleted INTEGER DEFAULT 0)',
        );
        await db.execute(
        ...
        await db.execute(
          'CREATE TABLE IF NOT EXISTS debt_payments(id INTEGER PRIMARY KEY AUTOINCREMENT, firestoreId TEXT UNIQUE, debtId INTEGER, debtFirestoreId TEXT, debtType TEXT, amount INTEGER, paidAt INTEGER, paymentMethod TEXT, note TEXT, createdBy TEXT, createdAt INTEGER, updatedAt INTEGER, isSynced INTEGER DEFAULT 0, shopId TEXT, personName TEXT, receivedBy TEXT, totalDebt INTEGER DEFAULT 0, alreadyPaid INTEGER DEFAULT 0, customerName TEXT, deleted INTEGER DEFAULT 0, paymentGroupId TEXT)',
        );
        await db.execute(
      ...
      onUpgrade: (db, oldV, newV) async {
        ...
        if (oldV < 106) {
          // Công nợ khách hàng gộp nhiều đơn: cột tùy chọn nhóm nhiều dòng
          // debt_payments thành 1 phiếu thu khi khách trả gộp cho nhiều đơn
          // cùng lúc. Nullable, đơn/khoản thu cũ không bị ảnh hưởng.
          try {
            await db.execute(
              'ALTER TABLE debt_payments ADD COLUMN paymentGroupId TEXT',
            );
          } catch (e) {
            debugPrint('DB upgrade error (debt_payments paymentGroupId): $e');
          }
        }
        if (oldV < 107) {
          // Máy cho khách mượn tạm trong lúc chờ sửa máy của họ. Nullable,
          // đơn cũ không bị ảnh hưởng.
          try {
            await db.execute('ALTER TABLE repairs ADD COLUMN loanerDevice TEXT');
          } catch (e) {
            debugPrint('DB upgrade error (repairs loanerDevice): $e');
          }
          try {
            await db.execute(
              'ALTER TABLE repairs ADD COLUMN loanerDeviceReturned INTEGER DEFAULT 0',
            );
          } catch (e) {
  ...
  Future<List<Product>> getInStockProducts() async {
    ...
    final maps = await (await database).query(
      'products',
      where:
          'shopId = ? AND (status = 1 OR status IS NULL) AND quantity > 0 AND (deleted = 0 OR deleted IS NULL)',
      whereArgs: [shopId],
      orderBy: 'createdAt DESC',
    );
    return List.generate(maps.length, (i) => Product.fromMap(maps[i]));
