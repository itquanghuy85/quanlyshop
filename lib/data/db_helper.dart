...
class DBHelper {
  ...
  Future<Database> _initDB() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
          'CREATE TABLE IF NOT EXISTS quick_input_codes(id INTEGER PRIMARY KEY AUTOINCREMENT, firestoreId TEXT UNIQUE, shopId TEXT, code TEXT, name TEXT, type TEXT, brand TEXT, model TEXT, capacity TEXT, color TEXT, condition TEXT, cost INTEGER, price INTEGER, description TEXT, supplier TEXT, paymentMethod TEXT, isActive INTEGER DEFAULT 1, createdAt INTEGER, lastModified INTEGER, isSynced INTEGER DEFAULT 0)',
        );
        await db.execute(
      ...
      onUpgrade: (db, oldV, newV) async {
        ...
        if (oldV < 23) {
          ...
          try {
            await db.execute(
              'CREATE TABLE IF NOT EXISTS quick_input_codes(id INTEGER PRIMARY KEY AUTOINCREMENT, firestoreId TEXT UNIQUE, shopId TEXT, name TEXT, type TEXT, brand TEXT, model TEXT, capacity TEXT, color TEXT, condition TEXT, cost INTEGER, price INTEGER, description TEXT, supplier TEXT, paymentMethod TEXT, isActive INTEGER DEFAULT 1, createdAt INTEGER, lastModified INTEGER, isSynced INTEGER DEFAULT 0)',
            );
          } catch (e) {
        ...
        if (oldV < 26) {
          ...
          try {
            ...
            await db.execute(
              'CREATE TABLE quick_input_codes_new(id INTEGER PRIMARY KEY AUTOINCREMENT, firestoreId TEXT UNIQUE, shopId TEXT, name TEXT, type TEXT, brand TEXT, model TEXT, capacity TEXT, color TEXT, condition TEXT, cost INTEGER, price INTEGER, description TEXT, supplier TEXT, paymentMethod TEXT, isActive INTEGER DEFAULT 1, createdAt INTEGER, lastModified INTEGER, isSynced INTEGER DEFAULT 0)',
            );
            // Copy data from old table to new table
            await db.execute(
              'INSERT INTO quick_input_codes_new SELECT id, firestoreId, shopId, name, type, brand, model, capacity, color, condition, cost, price, description, supplier, paymentMethod, isActive, createdAt, NULL, isSynced FROM quick_input_codes',
            );
            // Drop old table and rename new table
        ...
        if (oldV < 34) {
          ...
          try {
            await db.execute('CREATE INDEX IF NOT EXISTS idx_repairs_status_shop ON repairs(status, shopId)');
            await db.execute('CREATE INDEX IF NOT EXISTS idx_repairs_createdAt ON repairs(createdAt)');
            await db.execute('CREATE INDEX IF NOT EXISTS idx_products_name_brand ON products(name, brand)');
            ...
            await db.execute('CREATE INDEX IF NOT EXISTS idx_attendance_date ON attendance(dateKey)');
            debugPrint('Performance indexes created successfully');
          } catch (e) {
            debugPrint('DB upgrade error (indexes): $e');
          }
        }
        if (oldV < 35) {
          // Add shopId column to repairs table
          try {
            await db.execute('ALTER TABLE repairs ADD COLUMN shopId TEXT');
            debugPrint('Added shopId column to repairs table');
          } catch (e) {
            debugPrint('DB upgrade error (repairs shopId): $e');
          }
        }
      ...
      onOpen: (db) async {
        ...
        // Ensure lastModified column exists in quick_input_codes table
        try {
          final cols = await db.rawQuery('PRAGMA table_info(quick_input_codes)');
          final has = cols.any(
            (c) => (c['name'] ?? c['name'.toString()]) == 'lastModified',
          );
          if (!has) {
            await db.execute(
              'ALTER TABLE quick_input_codes ADD COLUMN lastModified INTEGER',
            );
            debugPrint('DB: added lastModified column to quick_input_codes');
          }
        } catch (e) {
          debugPrint('DB onOpen check error (quick_input_codes lastModified): $e');
        }
      },
    );
  }
  ...
  Future<List<Map<String, dynamic>>> getSupplierPaymentHistory(String supplierName) async {
    final db = await database;
    return await db.rawQuery('''
      SELECT
        p.id, p.amount, p.paidAt, p.paymentMethod, p.note, p.createdBy,
        'debt_payment' as paymentType,
        d.note as debtNote, d.type as debtType,
        NULL as orderCode
      FROM debt_payments p
      JOIN debts d ON p.debtId = d.id
      WHERE d.personName = ? AND d.type = 'OWE'

      UNION ALL

      SELECT
        po.id, po.totalCost as amount, po.createdAt as paidAt, 'CÔNG NỢ' as paymentMethod,
        'Thanh toán đơn nhập: ' || po.orderCode as note, po.createdBy,
        'purchase_settlement' as paymentType,
        NULL as debtNote, NULL as debtType,
        po.orderCode
      FROM purchase_orders po
      WHERE po.supplierName = ? AND po.paymentMethod = 'CÔNG NỢ' AND po.status = 'PAID'
  ...
  Future<int> getSupplierDebtAmount(String supplierName) async {
    final db = await database;
    
    // Tính tổng nợ từ purchase orders chưa thanh toán
    final purchaseDebtResult = await db.rawQuery(
      '''
      SELECT SUM(totalCost) as totalDebt
      FROM purchase_orders
      WHERE supplierName = ? AND paymentMethod = ? AND status = ?
      ''',
      [supplierName, 'CÔNG NỢ', 'PENDING'],
    );
    
    // Tính tổng nợ từ debts table
    ...
    int purchaseDebt = (purchaseDebtResult.first['totalDebt'] as int?) ?? 0;
    int debtAmount = (debtResult.first['totalDebt'] as int?) ?? 0;
    
    return purchaseDebt + debtAmount;
  }

  Future<Map<String, dynamic>> getSupplierDebtSummary(String supplierName) async {
    final db = await database;
    
    // Lấy thông tin nợ từ purchase orders
    final purchaseOrders = await db.query(
      'purchase_orders',
      where: 'supplierName = ? AND paymentMethod = ? AND status = ?',
      whereArgs: [supplierName, 'CÔNG NỢ', 'PENDING'],
      orderBy: 'createdAt DESC',
    );
    ...
    int totalPurchaseDebt = purchaseOrders.fold(0, (sum, order) => sum + (order['totalCost'] as int? ?? 0));
    int totalDebtAmount = debts.fold(0, (sum, debt) => sum + ((debt['totalAmount'] as int? ?? 0) - (debt['paidAmount'] as int? ?? 0)));
    int totalDebt = totalPurchaseDebt + totalDebtAmount;
    
    return {
      'totalDebt': totalDebt,
      'purchaseOrders': purchaseOrders,
      'debts': debts,
      'pendingPurchaseCount': purchaseOrders.length,
