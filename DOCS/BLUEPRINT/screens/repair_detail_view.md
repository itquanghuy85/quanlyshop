...
Nguồn code: lib/views/repair_detail_view.dart

## 1. Purpose
- Miền nghiệp vụ: Sửa chữa & Bảo hành.
...
- Widget chính: RepairDetailView, _PartsSelectionDialog, _PartsPaymentDialog
- Tín hiệu cấu trúc: list/grid = True, FAB = False, dialog = True, bottom sheet = True.
...
- Điều hướng nội bộ: file có khoảng 4 điểm Navigator.push, cho thấy mức độ điều phối đa màn hình.
...
- Quan hệ: màn hình nằm trong cụm Sửa chữa & Bảo hành và gắn với service domain tương ứng.
...
- navPush=4, FAB=False, Dialog=True, BottomSheet=True, ListOrGrid=True.
