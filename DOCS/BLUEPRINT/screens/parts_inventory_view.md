...
Nguồn code: lib/views/parts_inventory_view.dart

## 1. Purpose
- Miền nghiệp vụ: Kho & Sản phẩm.
...
- Widget chính: PartsInventoryViewContent, PartsInventoryView, _SupplierSearchField, _SupplierPickerSheet
- Tín hiệu cấu trúc: list/grid = True, FAB = True, dialog = True, bottom sheet = False.
...
- Điều hướng nội bộ: file có khoảng 5 điểm Navigator.push, cho thấy mức độ điều phối đa màn hình.
...
- Quan hệ: màn hình nằm trong cụm Kho & Sản phẩm và gắn với service domain tương ứng.
...
- navPush=5, FAB=True, Dialog=True, BottomSheet=False, ListOrGrid=True.
