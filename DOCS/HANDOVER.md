...
**⚠️ Chưa deploy:** `functions/index.js` (`sendBroadcastNotification` — chấp nhận thêm sentinel `auto:store`) đã sửa xong nhưng CHƯA `firebase deploy --only functions` — cần xác nhận riêng trước khi deploy production.
