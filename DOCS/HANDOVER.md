...
**⏳ ĐANG CHỜ USER DUYỆT PHƯƠNG ÁN (3 việc trong loạt yêu cầu 2026-08-29):**  
&nbsp;&nbsp;• **(2b) Thêm "nợ cũ" + "tổng nợ" + QR thanh toán vào phiếu gửi khách (Zalo/in)** — đụng layout ảnh biên nhận dùng chung sale+repair, cần chốt vị trí/chữ + nguồn QR (đã có `bank_qr_settings_view` + QR VietQR trong `[2026-08-23a]`). Chưa làm — chờ chốt phương án để tránh sửa phiếu khách hàng khi chưa xác nhận.  
&nbsp;&nbsp;• **(3) Thống kê "tiền ngân hàng chưa tất toán" ở Home / Tài chính** — số này đã tính sẵn trong `bank_installment_report_view.dart` (`totals['totalPending']` = Σ `loanAmount`+`loanAmount2` các đơn trả góp chưa tất toán). Cần chốt hiển thị ở đâu (Home dashboard / CashClosingView "tổng quan" / khung CẦN XỬ LÝ). Chưa làm.  
&nbsp;&nbsp;• **(4) Lỗi layout top-inset toàn cục (nhiều màn back/header chui vào status bar)** — ĐÃ ĐIỀU TRA XONG, chờ user xác nhận nguyên nhân trước khi sửa (yêu cầu rõ của user). Tóm tắt: app bị ép edge-to-edge (targetSdk 36 / Android 16, không opt-out được); patch toàn cục trong `main.dart` `MaterialApp.builder` (đọc `View.of(context).viewPadding.top`) KHÔNG hiệu quả vì (a) điều kiện `rawTop > mq.padding.top` và (b) **không rebuild khi engine gửi inset** (không có `WidgetsBindingObserver.didChangeMetrics`) → chụp giá trị 0 lúc frame đầu rồi đóng băng. Kiểm chứng trên máy: `InventoryView` (AppBar thường) back-btn y=176 ĐÚNG; `FastInventoryInputView` (AppBar + TabBar `bottom:`) back-btn y=131 SAI (~45px chui vào status bar 110px). Fix nhỏ nhất đề xuất: cho `_MyAppState` implement `WidgetsBindingObserver` + `didChangeMetrics()→setState`, và trong builder luôn lấy `max(mq.padding.top, viewPaddingTop)` (bỏ điều kiện `>`). ~8 dòng, 1 file, chỉ THÊM inset không bao giờ bớt → không regress màn đang đúng. (Band-aid trùng ở `dashboard_settings_view.dart:130` có thể xoá sau.)
