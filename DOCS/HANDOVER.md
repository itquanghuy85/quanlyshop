...
**⚠️ Lỗi TỰ GÂY RA rồi đã sửa ngay (2026-08-17b):** fix `[2026-08-17a]` ở trên có 1 lỗi logic khiến báo nhầm "mạng chập chờn" ở **MỌI đơn** dù mạng bình thường (do quên bật cờ `isSynced` trong DB khi bỏ qua bước gọi lại `syncAll()` để tối ưu) — user phát hiện ngay và báo lại trong vòng vài phút. Đã sửa + test lại ngay, xác nhận hết báo nhầm. Nhân tiện sửa luôn 1 lỗi có sẵn từ trước không liên quan: danh sách đơn sửa không tự cập nhật khi quay lại từ màn chi tiết (phải thoát hẳn ra Trang chủ mới đúng) — giờ luôn làm mới ngay khi quay lại. Chi tiết: `docs/CHANGELOG.md` mục `[2026-08-17b]`.
