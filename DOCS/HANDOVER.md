...
**Build Status:** ✅ `flutter build apk --debug` OK, đã cài + test trên Oppo CPH2203 (`m@m.com`/shop "M") — xem `[2026-08-29a]`, `[2026-08-24q]`, `[2026-08-24p]`, `[2026-08-24o]`, `[2026-08-24n]`, `[2026-08-24m]`, `[2026-08-24l]`, `[2026-08-24k]`, `[2026-08-24j]`, `[2026-08-24i]`, `[2026-08-24h]`, `[2026-08-24g]`, `[2026-08-24f]`, `[2026-08-24e]`, `[2026-08-24d]`, `[2026-08-24c]`, `[2026-08-24b]`, `[2026-08-24a]`, `[2026-08-23d]`, `[2026-08-23c]`, `[2026-08-23b]`, `[2026-08-23a]`, `[2026-08-22a]`  
...
&nbsp;&nbsp;• **(3) Home / CẦN XỬ LÝ** — `dashboard_cards.dart`: mục "trả góp NH" đổi nhãn → **"Tiền NH chưa tất toán: {tổng} đ · {N} đơn"** (query `SUM(loanAmount+loanAmount2)` cùng điều kiện query đếm đã có). **Chưa test trực quan** (shop "M" 0 đơn trả góp → mục không hiện, đúng logic); query là bản sao query đếm đang chạy production. Chi tiết: `[2026-08-29b]`.  
**🔴 (4) Lỗi layout top-inset toàn cục — FIX ĐỀ XUẤT ĐÃ THỬ VÀ HỎNG, ĐÃ REVERT:**  
&nbsp;&nbsp;Nguyên nhân (giữ nguyên): app bị ép edge-to-edge (targetSdk 36 / Android 16); `MediaQuery.padding.top` = 0 trên các màn `Scaffold`+`AppBar` **có `bottom:` (TabBar)** → toolbar/back-button chui sau status bar (110px). Kiểm chứng: `InventoryView` (AppBar thường) back-btn y=176 ĐÚNG; `FastInventoryInputView` (AppBar + TabBar) y=131 SAI.  
&nbsp;&nbsp;**Đã thử fix**: `_MyAppState implements WidgetsBindingObserver` + `didChangeMetrics()→setState(() {})` + builder luôn `math.max(mq.padding.top, viewPaddingTop)`. `flutter analyze` sạch, build OK — **NHƯNG cài lên máy thật app TREO ở màn "Đang kiểm tra phiên đăng nhập..."** (không vào được Home). Cô lập bằng cách stash riêng `main.dart` rồi build lại → app vào Home bình thường ⇒ **đúng là fix này gây treo**. Giả thuyết: `setState` trên `_MyAppState` mỗi lần `didChangeMetrics` ép rebuild toàn bộ `MaterialApp` liên tục lúc khởi động, cộng với vòng lặp lỗi sẵn có `CategoryService` / `shop_settings` INSERT-không-upsert (DB locked 10s) → livelock ở AuthGate. **ĐÃ revert hoàn toàn `main.dart` về HEAD.** Chưa có fix an toàn — cần hướng khác (vd. sửa ngay trong `CustomAppBar.build` nhánh có `bottom:`, hoặc bọc `MediaQuery` chỉ quanh AppBar đó, KHÔNG ép rebuild toàn app). **Chờ user chọn hướng.**  
**🐛 Bug sẵn có lộ rõ hơn khi test 2026-08-29:** `CategoryService.getShopSettings` → INSERT `shop_settings` fail `UNIQUE constraint failed: shop_settings.firestoreId` → clear cache → lặp lại, giữ DB locked ~10s nhiều lần lúc cold-start (Home mất ~45s mới hiện lần đầu sau cài mới). Không do thay đổi phiên này. Nên sửa thành upsert.
