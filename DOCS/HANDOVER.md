...
kẹt cờ `secure_frp_mode=1` (Android chặn share mọi app) — gỡ bằng `adb shell settings put secure
secure_frp_mode 0`, chưa chạy được (classifier chặn), cần user tự chạy. Chưa build release.
...
- Tất cả delta Tiền/Lãi/Nợ/Chốt quỹ đúng cho bán/sửa/kho/nợ/NH/linh kiện; máy nhân viên bán → máy
  ...
  tác/sửa chữa bị nuốt (UI báo thành công). **Cần rà shop thật** các phiếu "ĐÃ TRẢ NỢ" không có
  debt_payments. Còn treo: tab Lãi bỏ hẳn đơn CÔNG NỢ (khác Chốt quỹ) — xem CHANGELOG.
...
**trỏ ngược sang chính trang Nhắc nhở**; hai bên đếm khác nhau cho cùng một việc
(đơn sửa: thẻ `status IN (1,2)` vs service `status = 1`), và mục *"hàng chờ xác
nhận nhập kho"* của thẻ đếm `products.isPending` (kho tạm) trong khi màn mở ra là
danh sách phiếu `stock_entries` ⇒ **số không bao giờ khớp danh sách**. Nay
`ReminderService` là nguồn DUY NHẤT, thẻ chỉ vẽ 5 việc gấp nhất + "Xem tất cả".
Quá hạn công nợ **không tách mục riêng** (là tập con của đang nợ) mà nâng ưu tiên
+ ghi *"N quá hạn"*. Thợ không còn bị đếm 1 đơn thành 2 việc.
· **Sổ quỹ — đơn KẾT HỢP dồn hết vào ngân hàng:** `analyze()` có nhánh tách tiền
mặt/chuyển khoản nhưng 3/4 caller (Sổ quỹ, Trang chủ, Báo cáo tháng) lược mất cột
`cashAmount`/`transferAmount` ⇒ nhánh chết, cả đơn vào `bankIn`. Bán 10tr (4tr TM
+ 6tr CK) → quỹ tiền mặt **+0đ**. Đã cấp đủ cột cho cả 3; test hồi quy 5 case.
...
· **Lỗi 2:** `repairedBy` bị xoá trắng ⇒ đơn bị **ghi đè nguyên document** bằng bản chụp lúc tạo đơn từ máy còn giữ bản local cũ (`isSynced=0` — chính hệ quả của Lỗi 1). `_handleCreate` dùng `.set()` không merge và không có guard; guard của `_handleUpdate` chỉ chặn khi cloud **đã status 4** nên đơn "Chờ duyệt giao" (3) không được bảo vệ; 2 đường push hàng loạt của `SyncService` cũng đẩy nguyên `toMap()`. **Đã sửa:** luật chung `SyncOrchestrator.applyRepairCloudGuards()` áp cho cả 3 đường + `merge: true` cho create + không tự sinh status khi payload thiếu status.
· **Kiểm chứng:** `test/repair_progress_guard_test.dart` (4 ca) PASS, `flutter analyze` 3 file 0 error.
· **🔴 CHƯA NGHIỆM THU:** (a) chưa đọc được document thật trên Firestore để chốt đường ghi đè nào đã chạy (bị chặn quyền); (b) chưa diễn lại 2 máy (máy A tạo đơn có ảnh khi mạng yếu → máy B sửa xong + gửi duyệt → máy A online lại). **Đơn IPHONE 11 của khách BÉ THẮM trên production hiện vẫn đang sai trạng thái — cần sửa tay lại trong app.**

**🔑 AUDIT SUPER ADMIN (`[2026-09-05h]`) — KHÔI PHỤC QUYỀN SUPER ADMIN, báo cáo: `DOCS/AUDIT_SUPER_ADMIN_2026-09-05.md`.**
Hồi quy "đăng nhập super admin không vào thẳng trang Super Admin nữa" do **3 lỗi chồng nhau**, đã sửa 2, ghi nhận 1:
· **SA-09 (gốc rễ)** — `syncUserInfo` (`user_service.dart:1161`) ghi `role:'admin'` xuống Firestore mỗi lần đăng nhập, mà Cloud Function `buildCustomClaims()` đòi đúng chuỗi `'super_admin'` ⇒ **app tự thu hồi quyền super admin**, sửa tay trên Firestore Console bao nhiêu lần cũng vô ích. Bắt được đúng chuỗi trên máy: claims `super_admin` lúc 15:30:58 → `admin` lúc 15:37:20. Gốc rễ là `getUserRole()` map claims `super_admin` → `'admin'` (tên app-level) rồi chính tên đó bị dùng để ghi Firestore. **Đã sửa:** tách `persistedRole` (ghi Firestore) khỏi `resolvedRole` (tên app-level, giữ `'admin'`).
· **SA-10** — `main.dart:692` fast-path "cached mobile session" trả `isSuperAdmin: false` **cứng**, không đọc claims ⇒ super admin chỉ vào Console được **lần đăng nhập đầu**, từ lần 2 luôn rơi vào shop thường. **Đã sửa:** đọc claims trong fast path (token đã cache, không tốn vòng mạng) + `setCurrentUserSuperAdmin()`.
· **SA-08** — `home_view.dart:591` `final bool _isSuperAdmin = ...` là field initializer, chỉ tính 1 lần lúc dựng State trong khi quyền resolve muộn ⇒ 3 mục super admin có thể ẩn cả phiên. **Ghi nhận, chưa sửa.**
· **✅ NGHIỆM THU máy thật (CPH2239, `admin@huluca.com`):** vào **thẳng SUPER ADMIN CONSOLE**; Dashboard **147 shop / 146 hoạt động / 159 user / 1 shop bị khoá**; tab Shops lọc ACTIVE/LOCKED/đã xoá; tab Users lọc vai trò + tìm email trùng; tab Logs ghi thật; **4 vòng khởi động, có lần `syncUserInfo` chạy, quyền KHÔNG bị thu hồi**.
· **🔴 CÒN LẠI — 2 lỗi bảo mật cổng PIN chưa sửa:** **SA-03 (MEDIUM-HIGH)** cổng PIN **bị vượt hoàn toàn trên máy mới / sau khi xoá app data** vì `isPinSetup()` chỉ đọc SharedPreferences (trong khi `verifyPin()` đã có nhánh đọc Firestore); màn Cài đặt còn báo "Chưa thiết lập PIN" dù nhật ký có `pin_verified` 18–19/08. **SA-02 (MEDIUM)** PIN băm 1 vòng sha256 + salt hằng số dùng chung, PIN 4–6 số ⇒ ~1,11 triệu tổ hợp, bảng tra dựng sẵn phá được PIN của bất kỳ super admin nào. Thêm SA-04 (không giới hạn số lần thử PIN), SA-05 (`deleteShopSafe` không tự kiểm quyền — server vẫn chặn), SA-06 (màn từ chối là ngõ cụt).
· **Chưa chạy (cố ý):** khoá/xoá shop trên production (147 shop thật), idle guard 30 phút, đổi `_adminSelectedShopId`.
· **ĐIỂM ĐẠT kiến trúc:** claims ký server-side không giả mạo được · cache gắn uid · rules kiểm độc lập 30 chỗ · **tầng dữ liệu không có đường tắt** (`firestore_service.dart` 0 tham chiếu `isSuperAdmin`) · `admin_audit_log` bất biến thật · `admin_security` chỉ đọc được doc của chính mình.

**🧪 KIỂM THỬ TOÀN DIỆN TRƯỚC KHI LÊN STORE (`[2026-09-05f]`) — báo cáo đầy đủ: `docs/QA_FULL_REGRESSION_2026-09-05.md`.**
Chạy trên **2 máy Oppo thật** (CPH2203 Android 13 + CPH2239 Android 11), FFI/SQLite (`user_version=110`, `integrity_check ok`), Firestore đa thiết bị, và build release.
**Kết luận: READY WITH KNOWN ISSUES** — không có lỗi mất tiền / mất dữ liệu / lộ giá vốn / crash.
· **ĐẠT:** `flutter analyze lib` **0 error 0 warning** (1308 info); `flutter test` **+550 −8** (đúng 8 lỗi có sẵn, không hồi quy); APK + AAB release build xanh. Luồng tiền kiểm chứng từng đồng: nhập kho công nợ (SP 10 cái, nợ NCC 1.000.000) → bán 2 cái CÔNG NỢ cọc 100.000 (tồn 10→8, nợ 300.000/đã trả 100.000, `totalCost` 200.000) → đơn sửa 600.000 giao xong (status 1→3→4) → thu nợ 50.000 → **tổng dòng tiền ròng 1.32 Tr khớp chính xác sổ cái**; chặn thu quá hạn mức ("không được vượt 200.000") và sai mật khẩu quản lý đều đúng. **Xoá đơn bán hoàn nguyên chuẩn** (tồn 8→10, xoá nợ, ghi `SALE_VOID` −150.000 bù trừ về 0). **Đồng bộ 2 máy: toàn bộ dữ liệu test tự về máy 2, khớp 100%.** Xuất Excel bảng giá (3 sheet) + xem trước hoá đơn bán đều đúng.
· **ĐÃ SỬA 3 lỗi:** (1) ô SĐT khách hàng không kiểm định dạng — lưu được `phone=12` và đẩy lên cloud (vi phạm CLAUDE.md §5), nay gọi `UserService.validatePhone` và **đã nghiệm thu lại trên máy thật**; (2) `addRepair` thiếu `allowZero` ⇒ đơn sửa mới (`cost=0`) không ghi thẳng lên Firestore được; (3) `addProduct`/`updateProductCloud` cùng lỗi với hàng chưa định giá.
· **✅ ĐÃ SỬA + NGHIỆM THU (`[2026-09-05g]`, v3.5.1+555) — xoá khách hàng báo "Đã xóa khách hàng" nhưng khách quay lại.** `FirestoreService.deleteCustomerById` tìm document cloud bằng `.where('id', ...)` với **id autoincrement SQLite của máy đó** thay vì `firestoreId`; không khớp gì thì vòng `for` không chạy mà hàm vẫn `return true`. Hậu quả đo được: bản ghi bị `syncCustomersFromCloud` upsert đè lại sau ~40s (xoá 3 lần đều báo thành công, khách vẫn còn), và **máy thứ hai không hề biết đã xoá**. *(Cùng một khách có id cục bộ **25** trên máy 1 và **52** trên máy 2 ⇒ id cục bộ vốn không tra cloud được.)* **Đã sửa:** thêm `deleteCustomerByFirestoreId` xoá đúng `.doc(firestoreId)`; `deleteCustomerById` trả `false` khi không khớp; `CustomerService.deleteCustomer` tra `firestoreId` trước, đánh dấu `isSynced=0` rồi chỉ set `1` khi cloud xong, **trả `false` nếu cloud hụt**; `customer_management_view` trước đây bỏ qua kết quả xoá nay báo SnackBar đúng thật. **Nghiệm thu máy thật:** khách xoá xong `deleted=1` và **vẫn `deleted=1` sau 2 chu kỳ sync** (trước bật lại `0` trong ~40s); **máy 2 bản ghi biến mất hẳn**.
· **Ghi nhận thêm (không chặn):** `initRealTimeSync` chỉ subscribe **7** collection — `financial_activity_log`, `audit_logs`, `payment_intents`, `cash_closings`, `sales_returns`, `import_orders`, `price_catalog_items` chỉ về qua `downloadAllFromCloud`, mà hàm này tự bỏ qua khi realtime đang chạy ⇒ **máy thứ hai thấy số tiền cũ tới khi khởi động lại app** (đo được 1.2 Tr vs 1.35 Tr; restart xong cả hai đều 1.32 Tr — không mất dữ liệu). Thẻ "GIÁ THAM KHẢO" hiện Thu 600k/Vốn 150k/Lãi 300k nhìn như tính sai nhưng đúng (3 trung vị độc lập). Ô tìm kiếm chỉ lọc khi bấm Enter (`GlobalSearchBar.onChanged` thân rỗng).
· **✅ PHÂN QUYỀN GIÁ VỐN (CLAUDE.md §9) — nghiệm thu lại NHÁNH NHÂN VIÊN trên máy thật, ĐẠT CẢ 2 TẦNG.** Máy 2 đăng nhập `n@n.com` (`auth_cache_role=employee`, uid `1U6AlYLWJpbIBS5e4I1eKWUlTvV2`): (1) **UI** — quét toàn cây giao diện Bảng giá ra **0 lần** chữ "Vốn"/"Lãi" (cùng dòng đó chủ shop thấy `Thu 500.000đ · Vốn 333.333đ · Lãi 166.667đ`), Trang chủ không có khối "Dòng tiền hôm nay"; (2) **File Excel xuất ra** — `adb pull` cả 2 file rồi đọc thẳng `sharedStrings.xml`: sheet Sửa chữa/Bán hàng **10 cột → 8** (cắt `Giá vốn ĐX`, `Giá vốn NY`), sheet Bảng giá NCC **15 cột → 9** (cắt 4 cột giá vốn + `Nhà cung cấp` + `Ngày hoá đơn gần nhất`), toàn file **0 chuỗi chứa "vốn"** và không có tên NCC nào; giá thu khách vẫn giữ ⇒ nhân viên vẫn báo giá được. Đã trả máy 2 về `m@m.com`. **Rủi ro còn lại thuần là hồi quy: KHÔNG có test tự động nào canh quy tắc này.**
· **Dữ liệu test chưa dọn hết:** đơn sửa `rep_1788589291276_0900112233` (app **cấm xoá đơn ĐÃ GIAO** theo thiết kế), phiếu nhập `5cyYjhdvP1Wfg9cUq0tP` + nợ NCC 1.000.000 (giao diện chỉ cho "Thanh toán nợ" hoặc "Xoá NCC"), và 2 khách test do BUG-04. **Cố tình không sửa thẳng SQLite** vì sẽ lệch cloud và hỏng trạng thái đồng bộ.
...
**✅ ĐÓNG THÊM 4 MÓN (2026-09-05, đợt 2):**
· **`[2026-08-30v]`(4) Hệ số giá mùa vụ** — đặt +10%: banner "Đang áp hệ số mùa vụ: +10% (chỉ vào giá đề xuất) — chạm để sửa" hiện đúng, giá ĐỀ XUẤT tăng chuẩn (500k→550k, 1.5tr→1.65tr, 600k→660k), **giá Vốn KHÔNG đổi**, và **giá đã GHIM giữ nguyên 1.500.000đ** (không thành 1.65tr) ⇒ đúng "chỉ áp giá đề xuất". Trả về 0 thì banner mất, giá về 500k. ĐẠT.
· **`[2026-08-30w]`(3) Nút "Xem N đơn/SP tương ứng"** trong dialog ghim giá → mở đúng "ĐƠN SỬA TƯƠNG TỰ · 1 đơn dùng để tính giá tham khảo" kèm nút "Xem chi tiết". ĐẠT.
· **`[2026-08-30u]` Thẻ "GIÁ NIÊM YẾT" tự điền vào form tạo đơn sửa** — ghim thử IPHONE 8 · DV TÉT 4C = 777.000đ → mở Tạo đơn sửa, nhập model + lỗi → thẻ **"GIÁ NIÊM YẾT (Bảng giá) — Thu khách 777.000đ, Vốn 333.333đ"** hiện, ô giá **tự điền 777.000**, có nút "DÙNG GIÁ". Gõ lỗi KHÔNG DẤU ("DV TET 4C") vẫn khớp pin có dấu ⇒ chuẩn hoá dấu chạy đúng. Đã gỡ ghim sau khi test. ĐẠT.
· **`[2026-08-30v]`(3) Cảnh báo giá lệch >35%** — nhập 200.000 khi niêm yết 777.000 → hiện "Giá đang nhập **THẤP hơn 74%** so với…". ĐẠT.
· **`[2026-08-31c]` badge sync ảo** — `sync_queue` **TRỐNG**, AppBar luôn "Đã đồng bộ", **0 bản ghi `debts` xoá mềm chưa sync** (đúng nguồn gốc badge ảo), và 2 công nợ từng kẹt (`debt_partner_debt_rep_1787400740715…`, `debt_stock_vuLGqRt7…`) **không còn tồn tại ở local** ⇒ đã được dọn. ĐẠT.

**⚠️ `[2026-08-30b]` thẻ "💡 GIÁ THAM KHẢO" ở Tạo đơn bán — KHÔNG XÁC NHẬN ĐƯỢC.** Mở sheet "💰 Sửa giá bán sản phẩm" cho SP "IPHONE 128GB ĐEN 99" thì thẻ KHÔNG hiện. Nhiều khả năng là ĐÚNG logic (thẻ chỉ hiện khi model đó có lịch sử giá), nhưng kho shop test chỉ có 4 SP và **không SP nào có model từng bán trước đó** nên không dựng được ca dương tính. Cần một SP có lịch sử cùng model mới kết luận được. Phần khác của cùng đợt thì ĐẠT: dòng SP chưa có giá hiện cảnh báo cam **"⚠ Chưa định giá"**.

**📌 TỒN ĐỌNG CÒN LẠI — CHƯA LÀM:** `[2026-08-29g]` form bán hàng + edit dialog khoá PIN · `[2026-08-22a]` quét QR bằng app ngân hàng thật (cần app NH) · `[2026-08-16f]` luồng super admin (cần tài khoản super admin) · `[2026-08-08b]` Samsung A32 (không có máy) · **In** hoá đơn từng tab (không có máy in) · `[2026-08-30b]` thẻ GIÁ THAM KHẢO (thiếu dữ liệu, xem trên).
