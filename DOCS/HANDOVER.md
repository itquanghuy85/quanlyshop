...
· **🔴 CÒN 1 LỖI CHƯA SỬA — cần quyết trước khi phát hành:** **xoá khách hàng báo "Đã xóa khách hàng" nhưng khách quay lại.** `FirestoreService.deleteCustomerById` tìm document cloud bằng `.where('id', ...)` với **id autoincrement SQLite của máy đó** thay vì `firestoreId`; không khớp gì thì vòng `for` không chạy mà hàm vẫn `return true`. Hậu quả đo được: bản ghi bị `syncCustomersFromCloud` upsert đè lại sau ~40s (xoá 3 lần đều báo thành công, khách vẫn còn), và **máy thứ hai không hề biết đã xoá**. Đề xuất: tìm theo `firestoreId` + `return false` khi không khớp.
